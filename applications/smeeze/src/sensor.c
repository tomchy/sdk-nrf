...
int sensor_init(void)
{
	...
	err = battery_init();
	if (err) {
		LOG_ERR("Unable to initialize battery voltge measurements.");
	} else {
		err = battery_enable();
		if (err) {
			LOG_ERR("Unable to enable battery voltge measurements.");
		}
	}
...
int sensor_get_voltage(uint16_t *voltage_mv)
{
	int err = 0;
	int remaining_mv = 0;

	if (voltage_mv) {
		remaining_mv = battery_read_voltage_mv();
		*voltage_mv = remaining_mv;
		LOG_INF("Sensor    batt: %d [mV]", remaining_mv);
		LOG_INF("Charging  batt: %d", battery_is_charging());
	} else {
		LOG_ERR("NULL param");
		err = EINVAL;
	}
