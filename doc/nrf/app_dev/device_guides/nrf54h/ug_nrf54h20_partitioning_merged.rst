...
This guide describes the merged slot update strategy, guidelines for partitioning memory for this strategy, and an example of configuring an nRF54H20 application to use a merged slot for updates in the Direct XIP mode with revert.
...
For more details on the differences between update swap and direct XIP strategies, see :ref:`ug_bootloader_main_config`.
    ...
    Split slot (manifest, partial updates, image confirm - no need to do that for radio)
    bootloader requests / slot preference
    A/B sample - clarify app health check and auto confirm

Overview
********

The multi-core support in MCUboot design assumes that each core has its own, independent firmware image.
In such case, the non-volatile memory is partitioned into multiple slots, assigning two slots for each core.
Each binary that resides in each slot is wrapped with MCUboot metadata: the image header before the firmware binary and the image trailer with update flags after the firmware binary.
...
* Reduced flexibility in updating individual cores, as both firmwares need to be updated together.
...
The following diagram illustrates the memory map of the merged slot update strategy:
...
Both Direct XIP with and wihout revert modes support the merged slot strategy.

To convert an existing nRF54H20 application to use the merged slot update strategy, follow these steps:

* Create a common memory map devicetree fragment file (i.e. :file:`sysbuild/nrf54h20dk_nrf54h20_memory_map.dtsi`) that redefines all of the code as well as MCUboot partitions.
  ...
  - Radio core partitions: "cpurad_slot0_partition" and "cpurad_slot1_partition".
    ...
    To account for the trailer in the radio partition, include the radio image name (i.e. "ipc_radio" and "ipc_radio_secondary_app") in the "SB_CONFIG_MCUBOOT_IMAGES_ROM_END_OFFSET_AUTO" kconfig symbol.
...
* Include the modified memory map devicetree overlay file in the build of all images (bootloader, application core, radio core).
  This can be done by defining an overlay file under the :file:`sysbuild/<image_name>.overlay` for each image.

  For example, in the radio image overlay file (i.e. :file:`sysbuild/ipc_radio.overlay`) add:
  ...
  For example, in the aplication image overlay file (i.e. :file:`boards/<board_name>.overlay`) add:
...
* Configure the "zephyr,code-partition" property in the devicetree overlay for the bootloader to the first partition in the memory.
  ...
  For example, in the bootloader devicetree overlay file (i.e. :file:`sysbuild/mcuboot.overlay`) add:
...
* Configure the "zephyr,code-partition" property in the devicetree overlay for each image core to point to the new partition defined in the common memory map file.
  ...
  For example, in the application core devicetree overlay file (i.e. :file:`boards/<board_name>.overlay`) add:
...
* Optionally enforce building the merged binary in the build system.
  This can be done by setting the :kconfig:option:`SB_CONFIG_MCUBOOT_SIGN_MERGED_BINARY` Kconfig option to ``y`` in the :file:`sysbuild.conf` file.
...
* Build the application and radio core images as usual.
  This will produce the merged update candidate artifacts: ``build/zephyr.signed.bin`` and ``build/zephyr_secondary_app.signed.bin``.
...
* Send the opposite variant of the merged update candidate to the device i.e. using SMP protocol.
  For example, if the primary variant is currently running, send the ``build/zephyr_secondary_app.signed.bin`` file as the update candidate.
